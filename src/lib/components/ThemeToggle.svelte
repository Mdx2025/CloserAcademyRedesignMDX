...
<div class="flex items-center bg-secondary rounded-full p-1 relative overflow-hidden cursor-pointer">
	<svg class="absolute inset-0 w-full h-full pointer-events-none" aria-hidden="true" style="z-index: 0;">
		...
		<rect width="100%" height="100%" fill="transparent" filter="url(#toggle-blur)" rx="9999" />
	</svg>
	<button
		class="w-10 h-10 rounded-full flex items-center justify-center transition-colors relative z-10"
		class:bg-togactive={mode === 'light'}
		class:text-white={mode === 'light'}
		class:shadow-md={mode === 'light'}
		class:text-gray-500={mode !== 'light'}
		onclick={() => mode = 'light'}
	>
	...
	<button
		class="w-10 h-10 rounded-full flex items-center justify-center transition-colors relative z-10"
		class:bg-togactive={mode === 'dark'}
		class:text-white={mode === 'dark'}
		class:shadow-md={mode === 'dark'}
		class:text-gray-500={mode !== 'dark'}
		onclick={() => mode = 'dark'}
	>
